---
id: action-prof-aceleracao
title: Docência em Projeto de Aceleração da Aprendizagem
category: formacao
format: oficina
label: Pai Programa de Ação Integrada para o Aposentado
performed_by: '[[agent-rafael-semino]]'
my_role: Professor/Diretor
date_start: "2024"
attachments:
  - caption: "Docência em Projeto de Aceleração da Aprendizagem, imagem 1"
    role: documentation
    src: action-prof-aceleracao/action-prof-aceleracao-001.png
    type: image
---

Atuou como professor voluntário de teatro no PAI (Programa de Ação Integrada para o Aposentado), no ano de 2024. A ação formativa culminou na montagem "Barro Cru", encenada no Teatro José de Alencar, a qual contou com direção de Rafael Semino e texto de Walden Luiz.

![Docência em Projeto de Aceleração da Aprendizagem, imagem 1](../../media/images/action-prof-aceleracao/action-prof-aceleracao-001.png)

![Docência em Projeto de Aceleração da Aprendizagem, imagem 1](../../media/images/action-prof-aceleracao/action-prof-aceleracao-001.png)
