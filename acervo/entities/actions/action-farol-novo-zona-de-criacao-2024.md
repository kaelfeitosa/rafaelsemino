---
id: action-farol-novo-zona-de-criacao-2024
title: diretor | ator | roteirista | palestrante em Programa Zona de CriaÃ§Ã£o (YouTube)
kind: exibicao
performed_by: agent-coletivo-farol-novo
my_role: diretor | ator | roteirista | palestrante
work_id: ''
context:
  label: Programa Zona de CriaÃ§Ã£o (YouTube)
  kind: apresentacao | festival | mostra | residencia | oficina | premio
  location: null
  year: null
date_start: null
description: ''
attachments:
- type: image
  role: documentation
  src: record-zona-de-criacao-2024-001.jpeg
  caption: Destaque Zona de CriaÃ§Ã£o
---
Detalhes específicos da participação.
