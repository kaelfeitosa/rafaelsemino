---
attachments:
- caption: Projeto Abarca e Percurso Básico de Teatro
  role: documentation
  src: agent-projeto-abarca-001.png
  type: image
date_start: '2023'
description: Atuação como professor do Projeto Abarca e do Percurso Básico de Teatro,
  desenvolvendo processos pedagógicos voltados à formação artística em diferentes
  territórios, incluindo Itapipoca, Vicente Pinzón e Genibaú.
id: action-prof-percurso-basico
kind: formacao
my_role: Professor
performed_by: '[[agent-rafael-semino]]'
title: Professor em Percurso Básico de Teatro
work_id: ''
context_label: Percurso Básico de Teatro
context_kind: oficina
context_location: ''
---



Atuação como professor do Projeto Abarca e do Percurso Básico de Teatro, desenvolvendo processos pedagógicos voltados à formação artística em diferentes territórios, incluindo Itapipoca, Vicente Pinzón e Genibaú.


![Projeto Abarca e Percurso Básico de Teatro](../../media/images/agent-projeto-abarca-001.png)
